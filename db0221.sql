...
select 이름 from 학생 where 이름 not in (
	select 학생.이름 from 학생 inner join 수강신청 on 학생.학번=수강신청.학번
...
select 수강신청번호, concat(연도, 학기) '신청학년도' from 수강신청
where 학번=1801001;
...
select 수강신청번호, 학번, 날짜 from 수강신청
where 학번 = (select 학번 from 학생 where 학과=(select 학과번호 from 학과 where 학과명='컴퓨터정보학과'));
