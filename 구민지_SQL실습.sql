...
select 학번, substr(이름, 2, 2) as 이름 from 학생;
...
select count(수강신청번호) '과목수'from 수강신청내역 where 수강신청번호='1810002';
...
select count(*) '신청과목수', avg(평점) '평균 평점' from 수강신청내역
where 수강신청번호 = 1810001;
 
-- 57. <수강신청내역> 테이블에서 아래와 같이 과목별로 수강자 수를 읽어오는(과목번호, 수강자 수) SQL문을 작성하시오. 
select 과목번호, count(과목번호) '수강자수' from 수강신청내역
group by 과목번호;
...
select 과목번호, count(과목번호) '수강자수', avg(평점) '평균평점' from 수강신청내역
where 평점 <> -1
group by 과목번호;
...
select count(수강신청내역.과목번호) '과목수', sum(수강신청내역.평점) '평점합계' from 수강신청내역, 수강신청
