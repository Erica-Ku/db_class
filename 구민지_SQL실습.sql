...
select 과목명 from 학생, 수강신청, 수강신청내역, 과목
where 학생.학번=수강신청.학번 and 수강신청.수강신청번호=수강신청내역.수강신청번호 and
수강신청내역.과목번호=과목.과목번호 and 학생.이름='김민준' and 수강신청.연도=2018 and
수강신청.학기=1;
...
-- '과목번호', '과목', '시수'를 읽어오는 SQL문을 작성하시오.
select 과목번호, 과목명, 시수 from 과목
where 시수 in (1, 2, 3);

--  13. <교수> 테이블에서 '전화번호' 필드(열)의 값이 없는 레코드의 
-- '이름'과 '학과'을 읽어오는 SQL문을 작성하시오.
select 이름, 학과 from 교수
where 전화번호 is null;

-- 14. <수강신청> 테이블에서 날짜가 ‘2018-01-01’와 ‘2018-07-31’ 사이 
-- 수강신청한 레코드의 ‘학번’, ‘날짜’를 읽어오는 SQL문을 작성하시오.
select 학번, 날짜 from 수강신청
where 날짜>'2018-01-01' and 날짜<'2018-07-31';

-- 15. <교수> 테이블에서 '전화번호'가 있는 교수의 
-- '이름'과 ‘전자우편', ‘전화번호’를 읽어오는 SQL문을 작성하시오.
select 이름, 전자우편, 전화번호 from 교수
where 전화번호 is not null;

-- 16. <수강신청> 테이블에서 '학번'이 1601001'이고 '연도'가 2016'인 
-- 데이터를 가져오는 SQL문을 작성하시오.
select * from 수강신청
where 학번=1601001 and 연도=2016;

-- 17. <수강신청> 테이블에서 '학번'이 1601002'번과 1801002번인 학생의 
-- 수강신청 데이터를 가져오는 SQL문을 작성하시오.
select * from 수강신청
...
-- 20. <수강신청> 테이블에서 '1601001' 학생을 제외한 모든 학생의 수강신청 데이터를 
-- 가져오는 SQL문을 작성하시오.
select * from 수강신청
where 학번<>1601001;
...
-- 25. <수강신청내역> 테이블에서 '과목번호'가 'K20045'이거나 'K20056'이고 평점이 3인 레코드의 
...
select 학번, 이름, 시도 from 학생
where 학번 like '18%';

-- 29. <학생> 테이블에서 주소 중에 '시군구' 필드가 '구'로 끝나는 학생들의 
-- ‘학번’, '이름', '시도', '시군구'를 가져오는 SQL문을 작성하시오.
select 학번, 이름, 시도, 시군구 from 학생
where 시군구 like '%구';
...
select 학번, 학과, 이름, 시도 from 학생
where 이름 like '%서%';

-- 33. <학생> 테이블에서 '이름', '주소’, ‘시군구', '시도', '우편번호'의 정보를 묶어서 
-- 아래와 같이 하나의 필드처럼 보이도록 가져오는 SQL문을 작성하시오. (묶은 필드의 이름은 '학생정보'로 출력한다. '이름'으로 정렬하시오.)
select concat(이름,' ','(',우편번호,'  ',시도,'  ',시군구,'  ',주소,')') '학생정보'
from 학생
order by 이름;

-- 34. <과목> 테이블에서 '과목명', '담당교수' 필드를 가져와서 '담당교수' 필드의 별명을 
-- '담당교수사번'으로 바꾸어 출력하는 SQL문을 작성하시오.
select 과목명, 담당교수 '담당교수사번' from 과목;

-- 35. <수강신청> 테이블에서 아래와 같이 '학번'이 '1801001'인 학생의 수강신청 자료를 출력하도록 SQL문을 작성하시오.
select 수강신청번호, concat(연도,'학년도-',학기,'학기') '신청학년도' from 수강신청
where 학번=1801001;

-- 36. <학생> 테이블에서 '학번'과 '이름', 그리고 이름 중 성씨만 가져오는 SQL문을 작성하시오.
select 학번, 이름, substr(이름, 1, 1) '성' from 학생;
select 학번, 이름, left(이름, 1) '성' from 학생;

-- 37. <학생> 테이블에서 16학번인 학생의 학번과 이름, 학년을 가져오는 SQL문을 작성하시오.
select 학번, 이름, 학년 from 학생
where left(학번, 2) = 16;
-- where 학번 like '16%';

-- 38. <수강신청> 테이블에서 컴퓨터정보학과가 아닌 학생의 '수강신청번호', '학번','날짜'를 가져오는 SQL문을 작성하시오.
select 수강신청.수강신청번호, 수강신청.학번, 수강신청.날짜 from 수강신청, 학생, 학과
where 학과.학과번호=학생.학과 and 학과.학과명<>'컴퓨터정보학과' and 학생.학번 = 수강신청.학번;

-- 39. <수강신청> 테이블에서 2018년 3월에 수강 신청한 레코드의 '수강신청번호', 학번', '날짜'를 가져오는 SQL문을 작성하시오.
select 수강신청번호, 학번, 날짜 from 수강신청
-- where 날짜 like '%03%' and 연도=2018;
where year(날짜)=2018 and month(날짜)=03;

-- 40. <수강신청내역> 테이블에서 평점이 입력되어 있는(평점이 -1이 아닌) 레코드중 
-- '수강신청번호', '과목번호', '평점'과 평점이 0이면 '미취득' 아니면 '취득'으로 출력하는 SQL문을 작성하시오.
select 수강신청번호, 과목번호, 평점, if(평점=0,'미취득','취득') '취득여부' from 수강신청내역
where 평점 <> -1;

-- 41. <학생> 테이블에서 '이름'과 시도'를 하나로 묶어 필드명을 '이름'으로 읽어오는 SQL문을 작성하시오.
select concat(이름,'(',시도,')') '이름' from 학생;

-- 42. <과목> 테이블에서 ‘담당교수'와 '과목명'을 국문과 영문으로 읽어오는 SQL문을작성하시오.
select 담당교수, concat(과목명,' ','(',영문명,')') '과목명' from 과목
order by 과목명;

-- 43. <과목> 테이블에서 '과목명'과 학점', 그리고 한 학기를 15주라 가정했을 때 한 학기 총 수업 시간 수를 읽어오는 SQL문을 작성하시오.
select 과목명, 학점, (시수*15) '총시간수' from 과목
order by 과목명;

-- 44. <학생> 테이블에서 '학번'과 이름에서 성씨(last name)를 뺀 이름(first name)을 출력하는 SQL문을 작성하시오.
select 학번, substr(이름, 2, 2) '이름' from 학생;
select 학번, right(이름, 2) '이름' from 학생;
...
select 과목번호, count(*) '수강자 수', avg(평점) '평균평점' from 수강신청내역
where 평점 <> -1
group by 과목번호;
...
select 학생.학번, 학생.학과, 학생.이름, 학과.학과명 from 학생, 학과
where 학생.학과 = 학과.학과번호;
...
select 수강신청.수강신청번호, 수강신청.학번, 학생.이름 from 수강신청, 학생
-- where 수강신청.학번 = 학생.학번 and 날짜 LIKE '2018%';
where 수강신청.학번 = 학생.학번 and year(날짜)=2018
order by 수강신청번호;

-- 66. 학번이 1801001'인 학생이 수강 신청한 수강신청번호, 과목번호, 과목명을 출력하는 SQL 문을 작성하시오. 
select 수강신청내역.수강신청번호, 수강신청내역.과목번호, 과목.과목명 from 수강신청내역, 과목, 수강신청
where 수강신청내역.수강신청번호 = 수강신청.수강신청번호 and 수강신청.학번 = 1801001 and 수강신청내역.과목번호 = 과목.과목번호;

-- 67 = 66

-- 68. 학번이 '1801001'인 학생이 수강 신청한 과목 수와 평점 합계를 출력하는 SQL 문을 작성하시오.
select count(*) '과목수', sum(수강신청내역.평점) '평점합계' from 수강신청내역, 수강신청
where 수강신청내역.수강신청번호 = 수강신청.수강신청번호 and 수강신청.학번 = 1801001;
  
-- 69. 수강신청 별로 신청 학생의 학번, 이름, 평균 평점(평점합계/과목수)를 출력하는 SQL 문을 작성하시오. 
-- (단, 학점이 부여된 수강신청만 출력한다)
select 수강신청.수강신청번호, 수강신청.학번, 이름, sum(평점)/count(수강신청내역.과목번호) '평균평점'
...
-- 71. <수강신청내역> 테이블에서 수강신청번호 '1810001'와 '1610001'에 대하여 
-- 수강신청번호와 수강 과목번호, 수강 과목명을 출력하는 SQL 문을 작성하시오. 
select 수강신청내역.수강신청번호, 수강신청내역.과목번호, 과목.과목명 from 수강신청내역, 과목
where 수강신청내역.과목번호 = 과목.과목번호 and 수강신청내역.수강신청번호 in (1810001, 1610001)
order by 수강신청번호;
...
-- (단, 학점이 부여된 수강신청만 출력한다) 
select 수강신청.학번, 이름, count(*) '수강인원', avg(평점) '평균평점'
from 학생, 수강신청, 수강신청내역, 과목
where 학생.학번=수강신청.학번 and 수강신청.수강신청번호=수강신청내역.수강신청번호 and 수강신청내역.과목번호=과목.과목번호 and 평점 <> -1
group by 수강신청.학번;
